...
class CreateAppointmentUseCase(
    ...
    fun execute(
        ...
        val patient =
            patientRepository.findById(patientId)
                ?: throw ResourceNotFoundException(
                    "Patient not found in organization: $patientId"
                )
        ...
        if (patient.status != PatientStatus.ACTIVE) {
            throw ResourceStateConflictException(
                message =
                    "Patient is not active for appointment booking",
                code =
                    "PATIENT_NOT_ACTIVE",
                currentState =
                    patient.status.name
        ...
        if (practitioner.status != PractitionerStatus.ACTIVE) {
            throw ResourceStateConflictException(
                message =
                    "Practitioner is not active for appointment booking",
                code =
        ...
        val facilityService =
            facilityServiceRepository.findById(facilityServiceId)
                ?: throw ResourceNotFoundException(
                    "Facility service not found in facility: $facilityServiceId"
                )
        ...
        if (facilityService.status != FacilityServiceStatus.ACTIVE) {
            throw ResourceStateConflictException(
                message =
                    "Facility service is not active for appointment booking",
                code =
    ...
    private fun validatePractitionerAvailability(
        ...
        /*
         * An AVAILABLE exception can explicitly open a concrete
         * interval even when no recurring rule covers the period.
         *
         * The entire appointment must fit inside that interval.
         */
        ...
        /*
         * Recurring scheduling rules are same-day local-time
         * intervals. Cross-midnight bookings therefore require
         * an explicit AVAILABLE exception.
         */
