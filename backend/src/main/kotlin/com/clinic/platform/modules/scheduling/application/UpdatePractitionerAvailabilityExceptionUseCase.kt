...
@Service
class UpdatePractitionerAvailabilityExceptionUseCase(
    ...
    fun execute(
        ...
        if (
            existingException.organizationId != organizationId ||
            existingException.facilityId != facilityId ||
            existingException.practitionerId != practitionerId
        ) {
            throw ResourceNotFoundException(
                "Availability exception not found: $exceptionId"
            )
        ...
        val hasOverlap =
            updated.status == AvailabilityExceptionStatus.ACTIVE &&
                practitionerAvailabilityExceptionRepository
                    ...
                    .any { other ->
                        other.id != updated.id &&
                            ...
                            timeRangesOverlap(
                                firstStart = other.startAt,
                                firstEnd = other.endAt,
                                secondStart = updated.startAt,
                                secondEnd = updated.endAt
                            )
                    }
